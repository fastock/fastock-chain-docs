...
In faStock Exchange，any user can issue their own Token and Tokenpairs.
...
```shell
fastock-chain-cli tx token issue --from alice --symbol bcoin --total-supply 200000 --whole-name 'bcoin' --desc 'blockchain coin' -b block --mintable  
...
```shell
fastock-chain-cli tx dex list --base-asset tusdk-9a2 --quote-asset tbtc-965 --from mykey -b block -y
...
In order to make fair and open use of the matching resources of the blockchain, faStock-Chain allocates the system resources in the way of competitive ranking. Deposit OKT to a token pair can make the orders of this token pair matched earlier.


```shell
fastock-chain-cli tx dex deposit tusdk-9a2_tbtc-965 100okt --from mykey -b block -y
```
...
```shell
fastock-chain-cli tx dex withdraw tusdk-9a2_tbtc-965 50okt --from mykey -b block -y
```
