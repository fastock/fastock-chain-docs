...
```bash
$ fastock-chain-cli tx swap create-pair --token [token]
```
...
```shell
fastock-chain-cli tx swap create-pair --token eth-335 --from captain -y -b block --fees 0.002okt
{
  "height": "1243",
  ...
  "events": [
    {
      ...
      "attributes": [
        ...
        {
          "key": "sender",
          "value": "fastock-chain10q0rk5qnyag7wfvvt7rtphlw589m7frsku8qc9"
        }
      ]
    },
    {
...
```bash
$ fastock-chain-cli tx swap token --sell-amount [amount sellToken] --min-buy-amount [amount wantToken]
```
...
```shell
fastock-chain-cli tx swap token --sell-amount 100okt --min-buy-amount 80eth-335 --from captain -y -b block --fees 0.002okt
{
  "height": "1483",
  ...
  "events": [
    {
      ...
      "attributes": [
        ...
        {
          "key": "sender",
          "value": "fastock-chain1p6mshmwh5kz0g62pe6hghrjc696cyp7l0nf0st"
        },
        {
          "key": "module",
          "value": "ammswap"
...
```bash
$ fastock-chain-cli tx swap add-liquidity --max-base-amount [amount token] --quote-amount [amount okt] --min-liquidity [amount]
```
...
```shell
fastock-chain-cli tx swap add-liquidity --max-base-amount 10000eth-335 --quote-amount 10000okt --min-liquidity 0.001 --from captain -y -b block --fees 0.002okt
...
```bash
$ fastock-chain-cli tx swap remove-liquidity --liquidity [amount] --min-base-amount [amount token] --min-quote-amount [amount okt]
```
...
```shell
fastock-chain-cli tx swap remove-liquidity --liquidity 0.5 --min-base-amount 1eth-335 --min-quote-amount 1okt --from captain -y -b block --fees 0.002okt
{
  "height": "1620",
...
```bash
$ fastock-chain-cli query swap pool [token]
```
Example：
...
```bash
$ fastock-chain-cli query swap params
```
...
```bash
$ fastock-chain-cli query swap redeemable-assets [token] [amount]
```

Example：

```json
$ fastock-chain-cli query swap redeemable-assets eth-355 0.5
[
...
```bash
$ fastock-chain-cli query swap amount [amount sellToken] [wantToken]
```

Example：

```shell
fastock-chain-cli query swap amount 100eth-355 okt
"100.68709041"
```
