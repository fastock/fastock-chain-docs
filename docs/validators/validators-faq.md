...
We view testnet participation as a great way to signal to the community that you are ready and able to operate a validator. You can find all relevant information about the testnet [here](../getting-start/join-fastock-chain-testnet.html) and [here](https://github.com/fastock-chain/testnets).
...
- **Tendermint Key**: This is a unique key used to sign consensus votes.
  - It is associated with a public key `fastockvalconspub` (Get this value with `fastock-chain-daemon tendermint show-validator`)
  - It is generated when the node is created with fastock-chain-daemon init.
- **Application key**: This key is created from `fastock-chain-cli` and used to sign transactions. Application keys are associated with a public key prefixed by `fastockpub` and an address prefixed by `fastock-chain`. Both are derived from account keys generated by `fastock-chain-cli keys add`.

Note: A validator’s operator key is directly tied to an application key, but uses reserved prefixes solely for this purpose: `fastockvaloper` and `fastockvaloperpub`.
...
Validators and delegators on the faStock-Chain can vote on proposals to change operational parameters (such as the block gas limit), coordinate upgrades, or make a decision on any given matter.
...
In addition to running a faStock-Chain node, validators should develop monitoring, alerting and management solutions.

### What are bandwidth requirements?

The faStock-Chain network has the capacity for very high throughput compared to  Ethereum or Bitcoin.
...
The faStock-Chain team does not recommend one solution above the other. The community is encouraged to bolster the effort to improve HSMs and the security of key management.
