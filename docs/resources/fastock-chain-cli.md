...
# faStock-Chain Client

## faStock-Chain CLI

`fastock-chain-cli` is the tool that enables you to interact with the node that runs on the faStock-Chain network, whether you run it yourself or not. Let us set it up properly. In order to install it, follow the [installation procedure](../getting-start/install-fastock-chain.html).

## Setting up fastock-chain-cli

The main command used to set up `fastock-chain-cli` is the following:
...
# example: fastock-chain-cli config node http://fastock-chaintest-rpc1.fastockcn.com:26657 (testnet)
# example: fastock-chain-cli config node http://fastock-chain-rpc1.fastockcn.com:26657 (mainnet)
```

Or any of the following address ports:
```
# testnet
http://fastock-chaintest-rpc1.fastockcn.com:26657 (Hong Kong)
http://fastock-chaintest-rpc2.fastockcn.com:26657 (Ohio)
http://fastock-chaintest-rpc3.fastockcn.com:26657 (London)
http://fastock-chaintest-rpc1.fastock.com:26657 (Hong Kong. vpn required, if you are in mainland China)
http://fastock-chaintest-rpc2.fastock.com:26657 (Ohio. vpn required, if you are in mainland China)
http://fastock-chaintest-rpc3.fastock.com:26657 (London. vpn required, if you are in mainland China)

# mainnet
http://fastock-chain-rpc1.fastockcn.com:26657 (HongKong)
http://fastock-chain-rpc2.fastockcn.com:26657 (Tokyo)
http://fastock-chain-rpc1.fastock.com:26657 (HongKong. vpn required, if you are in mainland China)
http://fastock-chain-rpc2.fastock.com:26657 (Tokyo. vpn required, if you are in mainland China)
```
...
# mainnet
fastock-chain-cli config chain-id fastock-chain-66
```

Set the output format of fastock-chain-cli request response
```bash
fastock-chain-cli config output json    
fastock-chain-cli config indent true
```

## fastock-chain-cli user manual
...
```bash
fastock-chain-cli keys add <name> [flags]
```
#### Successful response:
```
fastock-chain-cli keys add wind
  
# Example output
{
  "name": "wind",
  "type": "local",
  "address": "fastock1ltxwpg2f2frsnq3et3qp7sfz2u44qsaj5ytlcf",
  "pubkey": "fastockpub1addwnpepqg8g82chdrf4ra4fn39e0lhzmds6qgs75emlzp2kqw5n69xt0fz3cewvx06",
...
fastock-chain-cli keys add --recover admin   -y -m "keen border system oil inject hotel hood potato shed pumpkin legend actor"
```
...
```bash
fastock-chain-cli keys list [flags]
```
#### Successful response:
```
  fastock-chain-cli keys list
  ...
  [
    {
      "name": "admin16",
      "type": "local",
      "address": "fastock1v853tq96n9ghvyxlvqyxyj97589clccr33yr7a",
      "pubkey": "fastockpub1addwnpepq2nfj9lqmqrwjyrqnp574tll493svxdltwlnqq3vn5ptmf2ceraesgyfegg"
    },
    {
      "name": "alice",
      "type": "local",
      "address": "fastock1vr9u0u829g68vcy6y362efm3tky4mhfxf28tth",
      "pubkey": "fastockpub1addwnpepq2nkjj2tv9yyvfaevc45n3tszsl9l7t642rh5j2udvwn5uee26gqv7vwhun"
    },
  ]
```
...
```bash
fastock-chain-cli keys show [name [name...]] [flags]
```
#### Successful response:
```
  fastock-chain-cli keys show bob
  
  # Example output
  {
    "name": "bob",
    "type": "local",
    "address": "fastock10487f9wxss2g2ctvpewkmjk543vg65x9rzv09n",
    "pubkey": 	  "fastockpub1addwnpepq2gaqy8nk0z0plexzusvs4g97wsvjtqftnpjkmw25lfh2hjz3wk0svf030h"
  }
```
...
```bash
fastock-chain-cli keys delete <name> [flags]
```
#### Successful response:
```
  fastock-chain-cli keys delete bob
...
```bash
fastock-chain-cli keys update <name> [flags]
```
#### Successful response:
```
  fastock-chain-cli keys update bob
...
```bash
fastock-chain-cli keys mnemonic [flags]
```
#### Successful response:
```
fastock-chain-cli keys mnemonic
...
#### Parameter description:
  ...
  | Parameter |                         Mark                          |
  | :-------: | :----------------------------------------------------------: |
  |  Address  | User address (eg. fastock10q0rk5qnyag7wfvvt7rtphlw589m7frsmyq4ya) |

#### Example:
Query user information, including serial numbers, public keys and token balances
```bash
fastock-chain-cli query account <address>
```
#### Successful response:
```
  fastock-chain-cli query account fastock10q0rk5qnyag7wfvvt7rtphlw589m7frsmyq4ya
  
  # Example output
  {
    "type": "auth/Account",
    "value": {
      "address": "fastock10q0rk5qnyag7wfvvt7rtphlw589m7frsmyq4ya",
      "coins": [
        {
          "denom": "acoin",
...
```bash
fastock-chain-cli tx token [command]
```
...
```bash
fastock-chain-cli tx send [from_key_or_address] [to_address] [amount] [flags]
```
...
```bash
fastock-chain-cli tx send alice fastock1jrhfgvmun4wd5qekxg2ma4xr405pn4dpwtx2qf 2okt --from alice --gas-prices 0.00000001okt --gas auto --gas-adjustment 1.5 -b block
```
#### Successful response:
```
{
...
Issue a new token
```bash
fastock-chain-cli tx token issue [flags]
```
#### Successful response:
```
fastock-chain-cli tx token issue --from alice --symbol bcoin -n 200000 -w 'bcoin' --desc 'blockchain coin' --gas-prices 0.00000001okt --gas auto --gas-adjustment 1.5 -b block --mintable  
# example  
{
  "height": "340949",
...
```bash
fastock-chain-cli tx token mint [flags]
```
#### Successful response:
```
fastock-chain-cli tx token mint --amount 10000000 --symbol okt --from alice --gas-prices 0.00000001okt --gas auto --gas-adjustment 1.5 -b block
# Example output
{
  "height": "341001",
...
```bash
fastock-chain-cli tx token burn [flags]
```
#### Successful response:
```
fastock-chain-cli tx token burn --from alice --symbol okt --amount 100.0 --gas-prices 0.00000001okt --gas auto --gas-adjustment 1.5 -b block
  # Example output
{
  "height": "341036",
...
```bash
  fastock-chain-cli query token info <symbol>
```
#### Successful response:
```
  # Example output
  {
    ...
    "wholeName": "bcoin",
    "totalSupply": "200000",
    "owner": "fastock10q0rk5qnyag7wfvvt7rtphlw589m7frsmyq4ya",
    "mintable": true
  }
```
...
```bash
fastock-chain-cli tx token multi-send [flags]
```
#### Successful response:

```
  fastock-chain-cli tx token multi-send --from alice --transfers '[{"to":"fastock106205vgqv4fnn0yqcq7y7j936pv4kznv99yw85","amount":"1okt,2btc-254"}]' --gas-prices 0.00000001okt --gas auto --gas-adjustment 1.5 -b block
...
Example:
```bash
fastock-chain-cli keys add --pubkey=fastockpub1addwnpepqtg367t3j6myh4ces0luq3f6g87ptzwszpl9g5r28tgavypkdmm2w5l4zuq p1

fastock-chain-cli keys add --pubkey=fastockpub1addwnpepqg334a4my6ufrs7r0ajsd6lxac9arsvtqljf0fzrgr27xvf3n5uugpsxna8 p2

fastock-chain-cli keys add --pubkey=fastockpub1addwnpepqd7jd60n88tk98hyh72xsw48pjpfhdw0cd77ju59eqc88sxscfjkgx7tyfc p3
```
...
```bash  
fastock-chain-cli keys add --multisig-threshold=2 --multisig=p1,p2,p3 p1p2p3
```
...
* Example:
```bash
fastock-chain-cli keys show -a p1p2p3
fastock-chain-cli tx send fastock1553hrs03kl2tlq47d9f6j477xdjp362l2cfetl 100.1okt --from=alice
fastock-chain-cli query account fastock1553hrs03kl2tlq47d9f6j477xdjp362l2cfetl
...
###### Example:
```bash
fastock-chain-cli tx send fastock1xd07r5a3e2mf4srqck3hvzww24c65hpt604ge5 10okt \
  --chain-id=fastock-chain-65 \
  --from=fastock1553hrs03kl2tlq47d9f6j477xdjp362l2cfetl \
  --generate-only > unsignedTx.json
```
##### p1, p2, p3 sign:
###### Example:
```bash
fastock-chain-cli tx sign \
  --multisig=fastock1553hrs03kl2tlq47d9f6j477xdjp362l2cfetl \
  --from=alice \
...
###### Example:
```bash 
fastock-chain-cli tx multisign \
  unsignedTx.json \
  p1p2p3 \
...
##### Example:
```bash
fastock-chain-cli tx token transfer-ownership [flags]
```
##### Successful response:
```
# from alice to jack
fastock-chain-cli tx token transfer-ownership --from fastock1pck0wndww84wtppc0vz9mcuvv7j5lcg00yf3gp --to fastock1x045ccxnwpurav2d5e25k25383qpmsr73293w0 --symbol okt
  ...
  "events": [
    {
      ...
      "attributes": [
        {
          "key": "sender",
          "value": "fastock1pck0wndww84wtppc0vz9mcuvv7j5lcg00yf3gp"
        },
        {
          "key": "module",
          "value": "token"
    ...
    },
    {
      ...
      "attributes": [
        {
          "key": "recipient",
          "value": "fastock-chain17xpfvakm2amg962yls6f84z3kell8c5llm79px"
        },
        {
          "key": "amount",
...
##### Example:
```bash
fastock-chain-cli tx token confirm-ownership [flags]
```
##### Successful response:
```
fastockcli tx token confirm-ownership --from fastock1x045ccxnwpurav2d5e25k25383qpmsr73293w0 --symbol okt
  ...
  "events": [
    {
      ...
      "attributes": [
        ...
        {
          "key": "sender",
          "value": "fastock1x045ccxnwpurav2d5e25k25383qpmsr73293w0"
        }
      ]
    },
    {
...
```bash
$ fastock-chain-cli tx dex  -h

Usage:
  fastock-chain-cli tx dex [command]
...
```shell
$ fastock-chain-cli tx dex list -h

Example:
    fastock-chain-cli tx dex list --base-asset tusdk-9a2 --quote-asset tbtc-965 --from mykey --gas-prices 0.00000001okt --gas auto --gas-adjustment 1.5 -b block -y

Usage:
    fastock-chain-cli tx dex list [flags]
...
```shell
$ fastock-chain-cli tx dex deposit -h

Example:
    fastock-chain-cli tx dex deposit tusdk-9a2_tbtc-965 100okt --from mykey --gas-prices 0.00000001okt --gas auto --gas-adjustment 1.5 -b block -y

Usage:
    fastock-chain-cli tx dex deposit [product] [amount] [flags]
...
```shell
$ fastock-chain-cli tx dex  withdraw -h

Example:
    fastock-chain-cli tx dex withdraw tusdk-9a2_tbtc-965 50okt --from mykey --from mykey --gas-prices 0.00000001okt --gas auto --gas-adjustment 1.5 -b block -y

Usage:
    fastock-chain-cli tx dex withdraw [product] [amount] [flags]
...
##### Example:
```bash
fastock-chain-cli tx dex transfer-ownership [flags]
```
##### Successful response:
```
# from alice to jack
fastock-chain-cli tx dex transfer-ownership --from fastock-chain1pck0wndww84wtppc0vz9mcuvv7j5lcg00yf3gp --to fastock-chain1x045ccxnwpurav2d5e25k25383qpmsr73293w0 --product btc_okt
...
##### Example:
```bash
fastock-chain-cli tx dex confirm-ownership [flags]
```
##### Successful response:
```
fastockcli tx dex confirm-ownership --from fastock1x045ccxnwpurav2d5e25k25383qpmsr73293w0 --product btc_okt
...
```shell
$ fastock-chain-cli query dex products -h

Example:
    fastock-chain-cli query dex products -p 1 -i 1

Usage:
    fastock-chain-cli query dex products [flags]
...
```shell
$ fastock-chain-cli query dex deposits -h

Example:
    fastock-chain-cli query dex deposits fastock1w7xcg4t90fxcpe95dzhykcvs9skzz2xsy60g3u -i 1

Usage:
    fastock-chain-cli query dex deposits [account-addr] [flags]
...
```shell
$ fastock-chain-cli query dex match-order -h

Example:
    fastock-chain-cli query dex match-order -i 1 --node ${url}

Usage:
    fastock-chain-cli query dex match-order [flags]
...
```shell
$ fastock-chain-cli query dex  params -h

Example:
    fastock-chain-cli query dex  params --node ${url}

Usage:
    fastock-chain-cli query dex params [flags]
...
```shell
$ fastock-chain-cli query dex  products-delisting -h

Example:
    fastock-chain-cli query dex  products-delisting --node ${url}

Usage:
    fastock-chain-cli query dex products-delisting [flags]
```
...
```bash
# place orders
fastock-chain-cli tx order new [flags]
```
```
Flags:
...
Global Flags:
  ...
  -e, --encoding string   Binary encoding (hex|b64|btc) (default "hex")
      --home string       directory for config and data (default "/Users/dwil/.fastock-chain-cli")
...
```
# example
fastock-chain-cli tx order new --product xxb-08a_okt,xxb-08a_okt -s BUY,BUY -p 1,1 -q 1,1 --from captain --gas-prices="0.00000001okt" --gas "200000" -y -b block
# example return
{
  "height": "373",
  ...
  "events": [
    {
      ...
      "attributes": [
        {
          "key": "sender",
          "value": "fastock10q0rk5qnyag7wfvvt7rtphlw589m7frsmyq4ya"
        },
        {
          "key": "sender",
          "value": "fastock10q0rk5qnyag7wfvvt7rtphlw589m7frsmyq4ya"
    ...
    },
    {
      ...
      "attributes": [
        {
          "key": "recipient",
          "value": "fastock183rfa8tvtp6ax7jr7dfaf7ywv870sykx3uvcxn"
        },
        {
          "key": "amount",
          "value": "1.00000000okt"
        ...
        {
          "key": "recipient",
          "value": "fastock17xpfvakm2amg962yls6f84z3kell8c5ljresa7"
        },
        {
          "key": "amount",
          "value": "0.00200000okt"
...
```bash
fastock-chain-cli tx order cancel [order-id] [flags]
```
```
Flags:
  -a, --account-number uint     The account number of the signing account (offline mode only)
...
```
# example
fastock-chain-cli tx order cancel ID0000000373-1,ID0000000373-2 --from captain --gas-prices="0.00000001okt" --gas "200000" -y -b block
...
```bash
fastock-chain-cli query order detail [orderId]
```
```
Flags:
...
```
# example
fastock-chain-cli query order detail ID0000000525-1
# example return
{
	"txhash": "B32EC706A2B5ACCF4FC43D6530588C0C9E36E79AC572D3443653281500D46FDA",
	"order_id": "ID0000000525-1",
	"sender": "fastock10q0rk5qnyag7wfvvt7rtphlw589m7frsmyq4ya",
	"product": "xxb-08a_okt",
	"side": "BUY",
	"price": "1.00000000",
	"quantity": "1.00000000",
...
##### Example:
```bash
fastock-chain-cli query order depthbook [product]
```
```
Flags:
...
```
# example
fastock-chain-cli query order depthbook mycoin_okt
# example return
{
	"asks": null,
	"bids": [{
...
##### Example:
```bash
fastock-chain-cli query order store
```
```
Flags:
...
#### Example:
```bash
fastock-chain-cli backend orders [open/closed] [addr] [flags]
...
```
# example
fastock-chain-cli backend orders open $(fastock-chain-cli keys show bob -a)
# example return
{
	"code": 0,
	"msg": "",
	...
	"data": {
		"data": [{
			...
			"sender": "fastock15uj03h8c2dluhfr9d9ele68v36cl3whjhgq2ch",
			"product": "mycoin_okt",
			"side": "SELL",
			"price": "11.10000000",
			"quantity": "1.00000000",
...
#### Example:
```bash
fastock-chain-cli backend deals [flags]
...
```
# example
fastock-chain-cli backend deals --address=$(fastock-chain-cli keys show bob -a)
# example return
{
	"code": 0,
	"msg": "",
...
#### Example:
```bash
fastock-chain-cli backend fees [addr] [flags]
...
```
# example
fastock-chain-cli backend fees $(fastock-chain-cli keys show bob -a)
# example return
{
	"code": 0,
	"msg": "",
	...
	"data": {
		"data": [{
			"address": "fastock15uj03h8c2dluhfr9d9ele68v36cl3whjhgq2ch",
			"fee": "0.00160000okt",
			"feeType": "deal",
			"timestamp": 1563890996
		}, {
...
#### Example:
```bash
fastock-chain-cli backend matches [flags]
...
```
# example
fastock-chain-cli backend matches
# example return
{
	"code": 0,
	"msg": "",
...
#### Example:
```bash
fastock-chain-cli backend txs [addr] [flags]
...
```
# example
fastock-chain-cli backend txs $(fastock-chain-cli keys show bob -a)
# example return
{
	"code": 0,
	"msg": "",
...
#### Example:
```bash
fastock-chain-cli backend block_tx_hashes [height] [flags]
...
```
# example
fastock-chain-cli backend block-tx-hashes 105
# example return
...
#### Example:
```bash
fastock-chain-cli backend klines [flags]
...
```
# example
fastock-chain-cli backend klines -p mycoin_okt 
...
#### Example:
```bash
fastock-chain-cli backend tickers [flags]
...
```
# example
fastock-chain-cli backend tickers -p mycoin_okt
...
```shell
$ fastock-chain-cli tx staking create-validator -h

Example:
    fastock-chain-cli tx staking create-validator --moniker $MONIKER --pubkey $PUBKEY --from mykey
    
Usage:
    fastock-chain-cli tx staking create-validator [flags]
...
```shell
$ fastock-chain-cli tx staking destroy-validator -h
  
Example:
    fastock-chain-cli tx staking destroy-validator --from mykey
    
Usage:
    fastock-chain-cli tx staking destroy-validator [flags]
...
```shell
$ fastock-chain-cli tx staking edit-validator -h

Example:
    fastock-chain-cli tx staking edit-validator --moniker "my new name" --identity "http://mynewwebsite/pic/newlogo.jpg" --website "http://mynewwebsites" --details "my new slogans" --from mykey
	
Usage:
    fastock-chain-cli tx staking edit-validator [flags]
...
```shell
$ fastock-chain-cli tx staking delegate -h

Example:
	  fastock-chain-cli tx staking delegate 1024.1024okt --from mykey
	  
Usage:
    fastock-chain-cli tx staking delegate [amount] [flags]
...
```shell
$ fastock-chain-cli tx staking vote -h

 
Example:
    fastock-chain-cli tx staking vote fastockvaloper1alq9na49n9yycysh889rl90g9nhe58lcs50wu5,fastockvaloper1svzxp4ts5le2s4zugx34ajt6shz2hg42a3gl7g,fastockvaloper10q0rk5qnyag7wfvvt7rtphlw589m7frs863s3m,fastockvaloper1g7znsf24w4jc3xfca88pq9kmlyjdare6mph5rx --from mykey

Usage:
    fastock-chain-cli tx staking vote [validator-addr1, validator-addr2, validator-addr3, ... validator-addrN] [flags]
```
...
```shell
$ fastock-chain-cli tx staking unbond -h

Example:
    fastock-chain-cli tx staking unbond 1024.1024okt --from mykey

Usage:
    fastock-chain-cli tx staking unbond [amount] [flags]
...
```shell
$ fastock-chain-cli query staking delegator -h

Example:
    fastock-chain-cli query staking delegator fastock1hw4r48aww06ldrfeuq2v438ujnl6alszzzqpph
    
Usage:
    fastock-chain-cli query staking delegator [address] [flags]
...
```shell
$ fastock-chain-cli query staking votes-to -h

Example:
	  fastock-chain-cli query staking votes-to fastockvaloper1alq9na49n9yycysh889rl90g9nhe58lcs50wu5
	  
Usage:
    fastock-chain-cli query staking votes-to [validator-addr] [flags]
...
```
Usage:
    fastock-chain-cli query staking [command] [flags]
...
```shell
$ fastock-chain-cli tx staking proxy reg -h

Example:
	  fastock-chain-cli tx staking proxy reg --from mykey
	  
Usage:
	  fastock-chain-cli tx staking proxy reg [flags]
...
```shell
$ fastock-chain-cli tx staking proxy unreg -h

Example:
	  fastock-chain-cli tx staking proxy unreg --from mykey
	  
Usage:
    fastock-chain-cli tx staking proxy unreg [flags]
...
```shell
$ fastock-chain-cli tx staking proxy bind -h

Example:
	  fastock-chain-cli tx staking proxy bind fastock1hw4r48aww06ldrfeuq2v438ujnl6alszzzqpph --from mykey
	  
Usage:
    fastock-chain-cli tx staking proxy bind [flags]
...
```shell
$ fastock-chain-cli tx staking proxy unbind -h

Example:
	  fastock-chain-cli tx staking proxy unbind fastock1hw4r48aww06ldrfeuq2v438ujnl6alszzzqpph --from mykey
	  
Usage:
    fastock-chain-cli tx staking proxy unbind [flags]
...
```shell
$ fastock-chain-cli query staking proxy -h

Example:
	  fastock-chain-cli query staking proxy fastock1hw4r48aww06ldrfeuq2v438ujnl6alszzzqpph
	  
Usage:
    fastock-chain-cli query staking proxy [address] [flags]
...
```shell
$ fastock-chain-cli tx distr withdraw-rewards -h

Example：
    fastock-chain-cli tx distr withdraw-rewards fastockvaloper1g7znsf24w4jc3xfca88pq9kmlyjdare6mph5rx --from ${name} --node ${url}

Usage: 
    fastock-chain-cli tx distr withdraw-rewards [validator-address] [flags]
...
```shell
$ fastock-chain-cli query distr params -h

Example：
    fastock-chain-cli query distr params --node ${url}
    
Usage: 
    fastock-chain-cli query distr params [flags]
...
```shell
$ fastock-chain-cli query distr community-pool -h

Example：
    fastock-chain-cli query distr community-pool --node ${url}
    
Usage: 
    fastock-chain-cli query distr community-pool [flags]
...
```shell
$ fastock-chain-cli query distr commission -h

Example：
    fastock-chain-cli query distr commission fastockvaloper1g7znsf24w4jc3xfca88pq9kmlyjdare6mph5rx --node ${url}
    
Usage: 
    fastock-chain-cli query distr commission [validator] [flags]
...
```shell
$ fastock-chain-cli tx gov submit-proposal delist-proposal -h

Submit a dex delist proposal along with an initial deposit.
The proposal details must be supplied via a JSON file.

Example:
  fastock-chain-cli tx gov submit-proposal delist-proposal <path/to/proposal.json> --from=<key_or_address>
...
Usage:
  fastock-chain-cli tx gov submit-proposal delist-proposal [proposal-file] [flags]
...
Global Flags:
      --chain-id string   Chain ID of tendermint node
  -e, --encoding string   Binary encoding (hex|b64|btc) (default "hex")
      --home string       directory for config and data (default "/Users/admin/.fastock-chain-cli")
  -o, --output string     Output format (text|json) (default "text")
      --passwd string     Pass word of sender (default "12345678")
      --trace             print out full stack trace on errors
```
...
```shell
$ fastock-chain-cli tx gov  submit-proposal -h
Submit a proposal along with an initial deposit.
Proposal title, description, type and deposit can be given directly or through a proposal JSON file.

Example:
  fastock-chain-cli tx gov submit-proposal --proposal="path/to/proposal.json" --from mykey
...
Which is equivalent to:
  ...
  fastock-chain-cli tx gov submit-proposal --title="Test Proposal" --description="My awesome proposal" --type="Text" \
	--deposit="10okt" --from mykey

Usage:
  fastock-chain-cli tx gov submit-proposal [flags]
  fastock-chain-cli tx gov submit-proposal [command]
...
Use "fastock-chain-cli tx gov submit-proposal [command] --help" for more information about a command.
```
##### Parameter description：
...
| --home              | account name and the directory where fastock-chain-cli is configured
if it is ~/.fastock-chain-cli can ignore the parameters |
...
```shell
$ fastock-chain-cli tx gov  submit-proposal param-change -h
...
Example:
  fastock-chain-cli tx gov submit-proposal param-change <path/to/proposal.json> --from=<key_or_address>
...
Usage:
  fastock-chain-cli tx gov submit-proposal param-change [proposal-file] [flags]
...
```shell
$ fastock-chain-cli tx gov deposit -h

Submit a deposit for an active proposal. You can
find the proposal-id by running "<appcli> query gov proposals".

Example:
  fastock-chain-cli tx gov deposit 1 10okt --from mykey

Usage:
  fastock-chain-cli tx gov deposit [proposal-id] [deposit] [flags]
...
```shell
$ fastock-chain-cli tx gov vote -h
...
Example:
  fastock-chain-cli tx gov vote 1 yes --from mykey

Usage:
  fastock-chain-cli tx gov vote [proposal-id] [option] [flags]
...
```shell
$ fastock-chain-cli tx gov submit-proposal community-pool-spend -h

Example:
	fastock-chain-cli tx gov submit-proposal community-pool-spend <path/to/proposal.json> --from=<key_or_address>

Where proposal.json contains:
{
  "title": "Community Pool Spend",
  "description": "Pay me some okt!",
  "recipient": "fastock5afhd6gxevu37mkqcvvsj8qeylhn0rz46zdlq",
  "amount": [
    {
      "denom": "okt",
      "amount": "10000"
...
Usage: 
    fastock-chain-cli tx gov submit-proposal community-pool-spend [proposal-file] [flags]
...
```shell
$ fastock-chain-cli query gov proposal -h
...
Usage:
  fastock-chain-cli query gov proposal [proposal-id] [flags]
...
```shell
$ fastock-chain-cli query gov proposals -h

Query for a all proposals. You can filter the returns with the following flags.

Example:
  fastock-chain-cli query gov proposals --depositor fastock1skjwj5whet0lpe65qaq4rpq03hjxlwd9nf39lk
  fastock-chain-cli query gov proposals --voter fastock1skjwj5whet0lpe65qaq4rpq03hjxlwd9nf39lk
  fastock-chain-cli query gov proposals --status (DepositPeriod|VotingPeriod|Passed|Rejected)

Usage:
  fastock-chain-cli query gov proposals [flags]
...
```shell
fastock-chain-cli query params params -h
...
Usage:
  fastock-chain-cli query params params [flags]
...
```sh
fastock-chain-cli tx slashing unjail -h
unjail a jailed validator:

Example:
    fastock-chain-cli tx slashing unjail --from mykey

Usage:
    fastock-chain-cli tx slashing unjail [flags]
Flags:
    --from string             Name or address of private key with which to sign
```
...
```sh
fastock-chain-cli query slashing signing-info -h

Use a validators' consensus public key to find the signing-info for that validator:

Example:
    fastock-chain-cli query slashing signing-info fastockvalconspub1zcjduepqfhvwcmt7p06fvdgexxhmz0l8c7sgswl7ulv7aulk364x4g5xsw7sr0k2g5

Usage:
    fastock-chain-cli query slashing signing-info [validator-conspub] [flags]
...
```sh
fastock-chain-cli query slashing params -h

Query genesis parameters for the slashing module:

Example:
    fastock-chain-cli query slashing params

Usage:
    fastock-chain-cli query slashing params [flags]
...
Set the fastock-chain-daemon log level  

```shell
$ fastock-chain-cli debug set-loglevel -h
...
Usage:
  fastock-chain-cli debug set-loglevel [flags]
...
Global Flags:
      --chain-id string   Chain ID of tendermint node
  -e, --encoding string   Binary encoding (hex|b64|btc) (default "hex")
      --home string       directory for config and data (default "/Users/yourname/.fastock-chain-cli")
  -o, --output string     Output format (text|json) (default "text")
      --passwd string     Pass word of sender (default "12345678")
      --trace             print out full stack trace on errors
```
...
```shell
$ fastock-chain-cli debug dump -h
Dump the data of kv-stores by a module name

Usage:
  fastock-chain-cli debug dump [module] [flags]
